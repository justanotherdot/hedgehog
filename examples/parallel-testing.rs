...
fn example_parallel_performance() {
    ...
    match result.outcome {
        TestResult::Pass { tests_run, .. } => {
            println!("✓ {tests_run} tests passed in {duration:?}");
            println!("  Speedup: {:.2}x", result.performance.speedup_factor);
            println!(
                "  Thread efficiency: {:.1}%",
                result.performance.thread_efficiency * 100.0
            );
        }
...
fn example_shared_state_testing() {
    ...
    match result.outcome {
        TestResult::Pass { tests_run, .. } => {
            println!("✓ {tests_run} concurrent increments completed successfully");
            println!("  Final counter value: {}", counter.get());
            println!(
                "  Thread failures: {}",
                result.concurrency_issues.thread_failures.len()
            );
            ...
            if actual == expected as i32 {
                ...
                println!("⚠ Potential race condition: expected {expected}, got {actual}");
            }
        }
        TestResult::Fail { counterexample, .. } => {
...
fn example_performance_comparison() {
    ...
    println!("Running {test_count} tests sequentially...");
    let start = Instant::now();
    ...
    println!("Running {test_count} tests in parallel (4 threads)...");
    let start = Instant::now();
    ...
    match (&sequential_result, &parallel_result.outcome) {
        (TestResult::Pass { .. }, TestResult::Pass { .. }) => {
            ...
            println!(
                "  Reported speedup: {:.2}x",
                parallel_result.performance.speedup_factor
            );
            println!(
                "  Thread efficiency: {:.1}%",
                parallel_result.performance.thread_efficiency * 100.0
            );
...
fn example_concurrent_testing() {
                ...
                if final_value > current {
                    ...
                    TestResult::Fail {
                        counterexample: format!(
                            "Counter didn't increase: {current} -> {final_value}"
                        ),
                        tests_run: 1,
                        shrinks_performed: 0,
                        property_name: Some("counter_increment".to_string()),
    ...
    let race_detected = racy_results
        .iter()
        .any(|r| !r.deterministic || r.race_conditions_detected > 0);
    let total_race_conditions = racy_results
        .iter()
        .map(|r| r.race_conditions_detected)
        .sum::<usize>();

    println!(
        "✓ Tested {} inputs with intentional race conditions",
        racy_results.len()
    );
    ...
    println!(
        "✓ Tested thread-safe counter from {} concurrent tests",
        safe_results.len()
    );
    ...
    let timeouts_detected = deadlock_results
        .iter()
        .filter(|r| r.timeout_detected)
        .count();
    let deadlock_info_count = deadlock_results
        .iter()
        .filter(|r| r.deadlock_info.is_some())
        .count();

    println!(
        "✓ Tested {} inputs for deadlock detection",
        deadlock_results.len()
    );
    ...
    println!(
        ...
        scenario_result.operation_results.len()
    );
    println!("  Execution time: {:?}", scenario_result.execution_time);
    println!(
        "  Constraints satisfied: {}",
        scenario_result.constraints_satisfied
    );

    // Show operation results
    for (op_name, result) in &scenario_result.operation_results {
        match result {
            TestResult::Pass { .. } => println!("  ✓ {op_name}: PASS"),
            TestResult::Fail { counterexample, .. } => {
                println!("  ✗ {op_name}: FAIL ({counterexample})")
            }
            _ => println!("  ? {op_name}: OTHER"),
        }
    }
    ...
    let explorer = hedgehog_core::interleaving_explorer(Gen::int_range(1, 10), {
        let counter = Arc::clone(&shared_counter);
        move |&increment| {
    ...
    println!(
        "✓ Explored interleavings for {} different inputs",
        interleaving_results.len()
    );

    let total_interleavings: usize = interleaving_results
        .iter()
        .map(|r| r.interleavings_explored)
        .sum();
    let total_races: usize = interleaving_results
        .iter()
        .map(|r| r.race_conditions_detected)
        .sum();
    let total_failures: usize = interleaving_results
        .iter()
    ...
    for (i, result) in interleaving_results.iter().enumerate() {
        if !result.deterministic {
            ...
            println!(
                "    {} out of {} interleavings failed",
                result.failed_interleavings, result.interleavings_explored
            );

            if let Some(pattern) = result.failing_patterns.first() {
                println!(
                    "    First failing pattern involved threads: {:?}",
                    pattern.threads_involved
                );
                println!("    Sequence length: {}", pattern.sequence.len());
            }
        } else {
            println!(
                "  ✓ Input {} was deterministic across all {} interleavings",
                i + 1,
    ...
    if total_races > 0 {
        println!(
            "✓ Successfully detected race conditions through systematic interleaving exploration!"
        );
    } else {
...
fn example_load_generation() {
    ...
    println!(
        "  Steady state: {:?}",
        result.phase_timings.steady_state_time
    );
    ...
    if total_requests == result.stats.operations_completed {
        ...
        println!(
            "⚠ Request counter mismatch: {} vs {}",
            total_requests, result.stats.operations_completed
        );
    }
    ...
    println!(
        "  Operations completed: {}",
        stress_result.stats.operations_completed
    );
    println!(
        "  Peak ops/sec: {:.1}",
        stress_result.stats.avg_ops_per_second
    );
    println!(
        "  Average response time: {:?}",
        stress_result.stats.avg_response_time
    );
    println!(
        "  P99 response time: {:?}",
        stress_result.stats.p99_response_time
    );
    ...
    if stress_ops_per_sec > load_ops_per_sec * 2.0 {
        println!(
            "✓ Stress test achieved {:.1}x higher throughput than rate-limited test",
            stress_ops_per_sec / load_ops_per_sec
        );
    } else {
        println!(
            "ℹ Stress test throughput: {stress_ops_per_sec:.1} ops/sec vs rate-limited: {load_ops_per_sec:.1} ops/sec"
        );
    }
    ...
    println!(
        "  Memory operations: {}",
        memory_result.stats.operations_completed
    );
    println!("  Success rate: {:.1}%", memory_result.success_rate * 100.0);
    println!(
        "  Avg response time: {:?}",
        memory_result.stats.avg_response_time
    );
    ...
    println!(
        "  Basic load test completed {} operations",
        result.stats.operations_completed
    );
    println!(
        "  Stress test achieved {:.1} ops/sec peak throughput",
        stress_result.stats.avg_ops_per_second
    );
    println!(
        "  Memory test processed {} allocations",
        memory_result.stats.operations_completed
    );
