...
use hedgehog_core::{data::*, gen::Gen, targeted::*};
...
fn expensive_computation(n: i32) -> i32 {
    ...
    std::thread::sleep(std::time::Duration::from_micros(
        (n.unsigned_abs() as u64).min(1000),
    ));
...
fn example_maximize_computation_time() {
    ...
    let test_function = |input: &i32| -> TargetedResult {
        ...
        if *input > 0 && result <= 0 {
            TargetedResult::Fail {
                counterexample: format!("input: {input}, result: {result}"),
                tests_run: 1,
                shrinks_performed: 0,
    ...
    println!(
        "  Best utility (computation time): {:.2} μs",
        stats.best_utility
    );
    ...
    match result {
        TargetedResult::Pass { utility, .. } => {
            println!("  Result: PASS with utility {utility:.2}");
        }
        TargetedResult::Fail {
            counterexample,
            utility,
            ...
            println!("  Result: FAIL with utility {utility:.2}");
...
fn example_find_parsing_errors() {
    ...
    let chars = "abcdefghijklmnopqrstuvwxyz0123456789null\x00\x01\x02"
        .chars()
        .collect::<Vec<_>>();
    ...
    let neighborhood = StringNeighborhood::new(
        "abcdefghijklmnopqrstuvwxyz0123456789null\x00\x01\x02"
            .chars()
            .collect(),
    );
...
fn example_minimize_cost_function() {
    ...
    fn cost_function(vec: &[i32]) -> f64 {
        ...
        let variance =
            vec.iter().map(|&x| (x as f64 - mean).powi(2)).sum::<f64>() / vec.len() as f64;
    ...
    let utility_function =
        |input: &Vec<i32>, _result: &TargetedResult| -> f64 { cost_function(input) };
...
fn example_compare_targeted_vs_random() {
    ...
    let test_function = |input: &i32| -> TargetedResult {
        ...
        match result {
            ...
            Err(_) => TargetedResult::Fail {
                counterexample: format!("input: {input} caused panic"),
                tests_run: 1,
                shrinks_performed: 0,
                property_name: Some("no_panic".to_string()),
                module_path: Some("targeted_testing::example_4".to_string()),
    ...
    match result {
        ...
        _ => {
            println!(
                "  Did not find the magic number, best utility: {:.2}",
                stats.best_utility
            );
        }
    }
