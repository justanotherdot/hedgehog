...
fn example_division_safety() {
    ...
    let prop = for_all(
        Gen::<(i32, i32)>::tuple_of(Gen::int_range(-100, 100), Gen::int_range(-10, 10)),
        |&(a, b)| {
    ...
    match prop.run(&Config::default().with_tests(50)) {
        TestResult::Pass { tests_run, .. } => {
            println!("   ✓ Division safety property passed {tests_run} tests");
...
fn example_string_parsing() {
    ...
    let prop = for_all(Gen::<String>::ascii_printable(), |s| {
    ...
    match prop.run(&Config::default().with_tests(100)) {
        TestResult::Pass { tests_run, .. } => {
            println!("   ✓ String parsing property passed {tests_run} tests");
...
fn example_integration_strategies() {
    ...
    let prop1 = for_all(Gen::int_range(20, 30), |&x| x > 0).with_examples(examples.clone());

    if let TestResult::Pass { tests_run, .. } = prop1.run(&Config::default().with_tests(8)) {
        println!("     ✓ Passed {tests_run} tests (examples tested first)");
    }
    ...
    if let TestResult::Pass { tests_run, .. } = prop2.run(&Config::default().with_tests(12)) {
        println!("     ✓ Passed {tests_run} tests (examples mixed throughout)");
    }
    ...
    if let TestResult::Pass { tests_run, .. } = prop3.run(&Config::default().with_tests(10)) {
        println!("     ✓ Passed {tests_run} tests (generated first, then examples)");
    }
    ...
    if let TestResult::Pass { tests_run, .. } = prop4.run(&Config::default().with_tests(10)) {
        println!("     ✓ Passed {tests_run} tests (examples only in first 3)");
    }
    println!();
}
...
fn example_regression_testing() {
    ...
    let prop = for_all(Gen::<Vec<i32>>::vec_of(Gen::int_range(-50, 50)), |data| {
        ...
        result.len() <= data.len() && result.iter().all(|&x| x >= 0 || x == i32::MIN)
    })
    ...
    match prop.run(&Config::default().with_tests(100)) {
        TestResult::PassWithStatistics {
            ...
            println!("   ✓ Regression testing passed {tests_run} tests");
