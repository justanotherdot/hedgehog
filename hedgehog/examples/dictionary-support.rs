...
fn example_dictionary_mixing() {
    ...
    println!("     Well-known: {} (expected ~70%)", well_known_count);
    println!("     Registered: {} (expected ~20-25%)", registered_count);
    println!("     Dynamic: {} (expected ~5-10%)", dynamic_count);
    println!();
}
...
fn example_web_domains() {
    ...
    let prop = for_all(
        Gen::<String>::web_domain(),
        |domain| is_plausible_url(domain)
    );
    
    match prop.run(&Config::default().with_tests(30)) {
        TestResult::Pass { tests_run, .. } => {
            println!("   ✓ All {} generated domains passed validation", tests_run);
...
fn example_http_testing() {
    ...
    let prop = for_all(
        Gen::<u16>::http_status_code(),
        |&status| {
            let category = response_category(status);
            category != "invalid"
        }
    ).classify("2xx_success", |&status| status >= 200 && status <= 299)
     .classify("4xx_client_error", |&status| status >= 400 && status <= 499) 
     .classify("5xx_server_error", |&status| status >= 500 && status <= 599)
    ...
    match prop.run(&Config::default().with_tests(100)) {
        TestResult::PassWithStatistics { tests_run, statistics, .. } => {
            println!("   ✓ All {} status codes were valid HTTP codes", tests_run);
            ...
            for (category, count) in &statistics.classifications {
                ...
                println!("     {}: {:.1}% ({} codes)", category, percentage, count);
            }
            ...
            if let Some(codes) = statistics.collections.get("status_code") {
                ...
                println!("     Range: {:.0}-{:.0}, Average: {:.1}", min, max, avg);
            }
        }
        TestResult::Fail { counterexample, .. } => {
...
fn example_database_testing() {
    ...
    fn build_select_query(table: &str, column: &str) -> std::result::Result<String, &'static str> {
        ...
        Ok(format!("SELECT {} FROM {}", column, table))
    }
    ...
    match prop.run(&Config::default().with_tests(50)) {
        TestResult::Pass { tests_run, .. } => {
            println!("   ✓ All {} SQL queries built successfully", tests_run);
            
            // Show example queries
            println!("   Example queries generated:");
            for _ in 0..3 {
                let table_tree = Gen::<String>::sql_identifier(false).generate(Size::new(10), Seed::random());
                let column_tree = Gen::<String>::sql_identifier(false).generate(Size::new(10), Seed::random());
...
fn example_network_testing() {
    ...
    fn validate_service_config(name: &str, port: u16) -> std::result::Result<String, &'static str> {
        ...
        match port {
            22 => if name != "ssh" { return Err("Port 22 reserved for SSH"); }
            80 => if name != "http" { return Err("Port 80 reserved for HTTP"); }
            443 => if name != "https" { return Err("Port 443 reserved for HTTPS"); }
            _ => {}
        }
    ...
    let service_names = vec![
        "ssh".to_string(), "http".to_string(), "https".to_string(),
        "api".to_string(), "web".to_string(), "app".to_string(),
    ];
    ...
    let prop = for_all(
     ...
     .classify("registered_port", |(_, port)| *port >= 1024 && *port <= 49151)
     .classify("dynamic_port", |(_, port)| *port >= 49152);
    
    match prop.run(&Config::default().with_tests(100)) {
        TestResult::PassWithStatistics { tests_run, statistics, .. } => {
            println!("   ✓ All {} service configurations handled correctly", tests_run);
            println!("   Port type distribution:");
            
            for (port_type, count) in &statistics.classifications {
                let percentage = (*count as f64 / tests_run as f64) * 100.0;
                println!("     {}: {:.1}% ({} configs)", port_type, percentage, count);
            }
