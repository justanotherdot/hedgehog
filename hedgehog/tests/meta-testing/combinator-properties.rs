...
pub fn test_map_composition() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            let size = Size::new(10);
            // Two separate maps
            let double_result = Gen::int_range(0, 20).map(|x| x + 1).map(|x| x * 2).generate(size, seed);
            
            // Single composed map
            let composed_result = Gen::int_range(0, 20).map(|x| (x + 1) * 2).generate(size, seed);
...
pub fn test_filter_predicate_correctness() {
    ...
    for i in 0..10 {
        ...
        if value >= 15 {
            panic!("Filter failed: generated {} which is not < 15", value);
        }
    }
...
pub fn test_map_filter_combination() {
    let prop = for_all_named(
        ...
        |&(size, seed): &(Size, Seed)| {
            // Map then filter
            let map_then_filter = Gen::int_range(0, 20).map(|x| x * 2)
                .filter(|&x| x < 30);
