...
pub fn test_property_scaling() {
    ...
    if time_ratio > count_ratio * 3.0 {
        panic!("Property evaluation doesn't scale linearly: {}x count increase led to {}x time increase", 
               count_ratio, time_ratio);
    }
    
    println!("✓ Property scaling property passed ({}x count -> {}x time)", count_ratio, time_ratio);
}
