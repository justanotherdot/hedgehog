...
pub fn test_concurrent_non_determinism_detection() {
    ...
    match prop.run(&fast_config) {
        TestResult::Pass { .. } => {
            println!("✓ Concurrent non-determinism detection property passed")
        }
...
pub fn test_deadlock_detection() {
    let prop = for_all_named(arbitrary_seed(), "seed", |&_seed: &Seed| {
        ...
        results.len() == 2 && results.iter().all(|r| r.execution_times.len() == 3)
        // All threads should complete or timeout
    });
...
pub fn test_concurrent_scenario_execution() {
    let prop = for_all_named(arbitrary_seed(), "seed", |&_seed: &Seed| {
        let scenario = concurrent_scenario("test_scenario")
            .operation("setup", |n: &i32| TestResult::Pass {
                tests_run: 1,
            ...
            .operation("cleanup", |_n: &i32| TestResult::Pass {
                tests_run: 1,
