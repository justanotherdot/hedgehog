...
pub fn test_recursive_composition() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let is_valid_tree = tree_result.iter().all(|&x| x >= 0 && x <= 10);
...
pub fn test_conditional_composition() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let in_small_range = result >= 1 && result <= 10;
            let in_large_range = result >= 100 && result <= 1000;
            let in_negative_range = result >= -50 && result <= -1;
...
pub fn test_deeply_nested_composition() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let complex_gen = Gen::<Option<std::result::Result<Vec<(i32, String)>, bool>>>::option_of(
                Gen::<std::result::Result<Vec<(i32, String)>, bool>>::result_of(
                    Gen::<Vec<(i32, String)>>::vec_of(
            ...
            match result {
                Some(Ok(vec)) => {
                    vec.iter().all(|(n, s)| {
                        *n >= -5 && *n <= 5 && s.chars().all(|c| c.is_ascii_alphabetic())
                    })
                }
