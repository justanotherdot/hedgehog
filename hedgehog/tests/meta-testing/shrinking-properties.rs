    ...
    let prop = for_all_named(
        ...
        |&(size, seed): &(Size, Seed)| {
            ...
            shrinks.into_iter().all(|shrink_value| {
                is_smaller_vector(shrink_value, &tree.value)
            })
        }
    );
...
pub fn test_shrinking_convergence() {
    let prop = for_all_named(
        ...
        |&(size, seed)| {
            ...
            if tree.value.is_empty() { return true; }
...
pub fn test_integer_shrinking_towards_zero() {
    let prop = for_all_named(
        ...
        "initial_value", 
        |&initial_value: &i32| {
            if initial_value == 0 { return true; }
            ...
            if shrinks.is_empty() { return true; }
            
            // At least one shrink should be closer to zero
            shrinks.into_iter().any(|shrink_value| {
                shrink_value.abs() <= initial_value.abs()
            })
        }
    );
...
pub fn test_string_shrinking_shorter() {
    let prop = for_all_named(
        ...
        |initial_string: &String| {
            if initial_string.is_empty() { return true; }
            ...
            shrinks.into_iter().any(|shrink_value| {
                shrink_value.len() < initial_string.len()
            })
        }
    );
    ...
    let fast_config = Config::default().with_tests(15).with_shrinks(5);
    match prop.run(&fast_config) {
        TestResult::Pass { .. } => println!("✓ String shrinking produces shorter strings property passed"),
...
pub fn test_shrinking_preserves_failures() {
    ...
    let prop = for_all_named(
        ...
        move |initial_vec: &Vec<i32>| {
            if failing_property(initial_vec) { return true; } // Skip passing cases
