        ...
        match Gen::frequency(choices) {
            Ok(freq_gen) => {
                let freq_property = for_all(
                    freq_gen,
                    |&x: &i32| (0..=100).contains(&x), // Should always be true
                );
...
pub fn test_one_of_generator_workflow() {
    let prop = for_all_named(arbitrary_seed(), "seed", |&_seed: &Seed| {
        ...
        match Gen::one_of(generators) {
            Ok(oneof_gen) => {
                let oneof_property = for_all(oneof_gen, |&x: &i32| {
                    // Value should be from one of the ranges
                    (0..=10).contains(&x) || (20..=30).contains(&x) || (40..=50).contains(&x)
                });
...
pub fn test_complex_nested_workflow() {
    let prop = for_all_named(arbitrary_seed(), "seed", |&_seed: &Seed| {
        ...
        let nested_property = for_all(
            Gen::vec_of(Gen::<(i32, String)>::tuple_of(
                Gen::int_range(0, 50),
                Gen::<String>::ascii_alpha(),
            )),
            |pairs: &Vec<(i32, String)>| {
                ...
                pairs
                    .iter()
                    .all(|(num, string)| *num < 100 && !string.is_empty())
            },
        );
...
pub fn test_custom_config_workflow() {
    let prop = for_all_named(arbitrary_seed(), "seed", |&_seed: &Seed| {
        ...
        configs
            .into_iter()
            .all(|config| matches!(simple_property.run(&config), TestResult::Pass { .. }))
    });
