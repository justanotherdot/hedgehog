...
pub fn test_exponential_distribution_bias() {
    ...
    if lower_quartile_ratio < 0.5 {
        panic!("Exponential distribution not strongly biased: {:.2}% in lower quartile", lower_quartile_ratio * 100.0);
    }
    
    println!("✓ Exponential distribution bias property passed ({:.1}% in lower quartile)", lower_quartile_ratio * 100.0);
}
