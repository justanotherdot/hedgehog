...
pub fn test_character_generator_ranges() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let print_valid = print_char.is_ascii() && print_char >= ' ' && print_char <= '~';
...
pub fn test_string_generator_character_sets() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let alnum_string = Gen::<String>::ascii_alphanumeric().generate(size, seed).value;
            ...
            let print_valid = print_string.chars().all(|c| c.is_ascii() && c >= ' ' && c <= '~');
...
pub fn test_string_range_constraints() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let ranged_string = Gen::<String>::alpha_with_range(range).generate(size, seed).value;
            ...
            let linear_string = Gen::<String>::alphanumeric_with_range(linear_range).generate(size, seed).value;
...
pub fn test_string_shrinking_behavior() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let all_shrinks_valid = shrinks.iter().all(|s| 
...
pub fn test_character_simplification() {
    let prop = for_all_named(
        ...
        |&_seed: &Seed| {
            ...
            let uppercase_simplifies = 'Z' != 'Z'.to_ascii_lowercase();
            ...
            uppercase_simplifies && numbers_work && has_uppercase && has_special
        }
    );
...
pub fn test_email_address_generation() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let valid_structure = parts.len() == 2 && 
                !parts[0].is_empty() && 
                !parts[1].is_empty();
...
pub fn test_sql_identifier_generation() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let risky_valid = !risky_id.is_empty() && 
                risky_id.chars().all(|c| c.is_ascii_alphabetic());
...
pub fn test_string_generation_variety() {
    let prop = for_all_named(
        ...
        |&_seed: &Seed| {
            ...
            // All should be valid
            let all_valid = strings.iter().all(|s| 
...
pub fn test_unicode_edge_cases() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let ascii_only = ascii_string.chars().all(|c| c.is_ascii());
        ...
        |&seed: &Seed| {
            ...
            match hex_char_gen {
                Ok(char_gen) => {
                    ...
                    let hex_valid = hex_string.chars().all(|c| 
                        c.is_ascii_hexdigit() && c.is_ascii_uppercase() || c.is_ascii_digit()
                    );
