        ...
        |&seed: &Seed| {
            ...
            match &tree.value {
                Some(_) => {
                    ...
                    let has_none_shrink = shrinks.contains(&&None);
...
pub fn test_nested_option_result_combinations() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let option_result_gen = Gen::<Option<std::result::Result<i32, String>>>::option_of(
                Gen::<std::result::Result<i32, String>>::result_of(
                    Gen::int_range(1, 10),
                    Gen::<String>::ascii_alpha(),
                )
            );
...
pub fn test_complex_type_option_result() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let vec_option_gen = Gen::<Option<Vec<i32>>>::option_of(
                Gen::<Vec<i32>>::vec_of(Gen::int_range(1, 20))
            );
            
            let vec_option = vec_option_gen.generate(size, seed).value;
            let vec_option_valid = match vec_option {
                Some(vec) => vec.iter().all(|&n| n >= 1 && n <= 20),
                None => true,
            };
            ...
            let tuple_result_valid = match tuple_result {
                Ok((n, s)) => n >= -10 && n <= 10 && s.chars().all(|c| c.is_ascii_alphabetic()),
...
pub fn test_constrained_option_result_shrinking() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let constrained_option_gen = Gen::<Option<i32>>::option_of(
                Gen::int_range(100, 200)
            );
            ...
            let constrained_result_gen = Gen::<std::result::Result<bool, i32>>::result_of(
                Gen::bool(),
                Gen::int_range(400, 500)
            );
    ...
    match prop.run(&fast_config) {
        TestResult::Pass { .. } => println!("✓ Constrained Option/Result shrinking property passed"),
...
pub fn test_option_result_combinator_composition() {
    let prop = for_all_named(
        ...
        |&seed: &Seed| {
            ...
            let mapped_option_gen = Gen::<Option<i32>>::option_of(Gen::int_range(1, 10))
            ...
            let mapped_option_valid = match mapped_option {
                Some(n) => n >= 2 && n <= 20 && n % 2 == 0, // Should be even, doubled
                None => true,
            };
    ...
    let fast_config = Config::default().with_tests(15).with_shrinks(2);
    match prop.run(&fast_config) {
        TestResult::Pass { .. } => println!("✓ Option/Result combinator composition property passed"),
