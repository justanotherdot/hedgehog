...
fn test_new_integer_from_range_apis() {
    ...
    for _ in 0..10 {
        ...
        assert!((-10..=10).contains(&i8_val));

        let i16_val = i16_gen.generate(size, seed).value;
        assert!((-1000..=1000).contains(&i16_val));

        let isize_val = isize_gen.generate(size, seed).value;
        assert!((-50..=50).contains(&isize_val));
        ...
        assert!((100..=1000).contains(&u16_val));
...
fn test_new_tuple_apis() {
    ...
    assert!((-5..=5).contains(&i8_val));
    assert!((-50..=50).contains(&i16_val));
    assert!((-500..=500).contains(&i32_val));
    assert!((-5000..=5000).contains(&i64_val));
    assert!((-50000..=50000).contains(&isize_val));
}
...
fn test_shrinking_with_new_apis() {
    ...
    if i8_tree.value != 0 {
        assert!(
            !i8_shrinks.is_empty(),
            "i8 should have shrinks when not at origin"
        );
    }
    ...
    if i8_val != 0 || i16_val != 0 || i32_val != 0 {
        assert!(
            !triple_shrinks.is_empty(),
            "Triple should have shrinks when not at origin"
        );
    }
}
