...
impl<T> Tree<T> {
    ...
    where
        F: Fn(T) -> Tree<U>,
    {
        let Tree { value: new_value, children: new_children } = f(self.value);
        
        let mapped_children: Vec<Tree<U>> = self.children
            .into_iter()
    ...
    fn expand_recursive<'a>(&'a self, result: &mut Vec<&'a T>, max_depth: usize, current_depth: usize) {
        ...
        F: Fn(&T) -> bool + Clone,
    {
        ...
        let filtered_children: Vec<Tree<T>> = self.children
            .into_iter()
...
mod tests {
    ...
    fn test_tree_with_children() {
        let tree = Tree::with_children(
            10,
            vec![Tree::singleton(5), Tree::singleton(0)]
        );
