...
pub struct SimulatedAnnealing<T> {
    ...
    utility_function: Box<dyn Fn(&T, &TargetedResult) -> f64>,
    /// Function that tests the property
    test_function: Box<dyn Fn(&T) -> TargetedResult>,
    ...
    T: 'static + std::fmt::Debug + Clone,
{
    ...
    pub fn search(&self, test_config: &Config) -> (TargetedResult, SearchStats) {
        ...
        while step < self.config.search_steps 
            && temperature > self.config.min_temperature 
        {
            ...
            // Generate neighbor
            if let Some(neighbor) = self.neighborhood.neighbor(&current_input, temperature, &mut rng) {
...
where
    ...
    N: NeighborhoodFunction<T> + 'static,
{
    SimulatedAnnealing::new(generator, utility_function, test_function, neighborhood, config)
}
...
impl NeighborhoodFunction<f64> for FloatNeighborhood {
    fn neighbor(&self, input: &f64, temperature: f64, rng: &mut dyn RngCore) -> Option<f64> {
        ...
        let temp_factor = (temperature / 100.0).min(1.0).max(0.001);
    ...
    fn neighbor(&self, input: &Vec<T>, temperature: f64, rng: &mut dyn RngCore) -> Option<Vec<T>> {
        ...
        for element in result.iter_mut() {
            if rng.gen::<f64>() < self.modification_probability {
                if let Some(new_element) = self.element_neighborhood.neighbor(element, temperature, rng) {
                    *element = new_element;
                    modified = true;
                }
            }
        ...
        if modified {
            ...
            if let Some(new_element) = self.element_neighborhood.neighbor(&input[index], temperature, rng) {
                let mut result = input.clone();
